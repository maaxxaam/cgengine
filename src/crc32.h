...
#ifndef OPENAWE_CRC32_H
#define OPENAWE_CRC32_H
...
/*
 * CRC32 table taken from
 * http://web.mit.edu/freebsd/head/sys/libkern/crc32.c
 */
static constexpr uint32_t kCRC32Table[] = {
...
/*!
 * Calculate the crc32 hash from a string
...
constexpr uint32_t crc32(const std::string_view &data) {
	...
	for (char date : data) {
		hash = kCRC32Table[(hash ^ date) & 0xFF] ^ (hash >> 8);
	}
 ...
 * \return The hash caculated from the given string
 */
constexpr uint32_t crc32(const unsigned char *data, size_t length) {
	uint32_t hash = 0xFFFFFFFF;
